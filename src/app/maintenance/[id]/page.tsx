...
import { ArrowLeft, User, Home, Calendar, AlertTriangle, Play, Square, Timer as TimerIcon, Bell, Wrench, CheckCircle, Phone, Send } from 'lucide-react';
...
export default function MaintenanceDetailPage() {
  ...
  const initialRequest = maintenanceRequests.find((r) => r.id === requestId);
  ...
  // When the global state changes, update the local state.
  useEffect(() => {
  ...
  const updateGlobalRequestState = (updatedRequest: any) => {
    ...
    // This now updates the local state, which is fine for this page.
    // To reflect changes globally, a more robust state management (like Zustand or Context) would be needed.
    // For this prototype, we are directly mutating the imported `allMaintenanceRequests` to simulate global state.
  ...
  // A worker can see a request if they are assigned OR if it's unassigned on a property they manage
  if (user.role === 'worker') {
        ...
        <div className="md:col-span-2 space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                A detailed look at the maintenance request.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="space-y-2">
                ...
                <p className="text-muted-foreground">
                  A more detailed description of the issue would go here. For now, we are just displaying the title of the issue again: {request.issue}. This could include photos or more specific information provided by the tenant.
                </p>
              </div>
          ...
          {(user.role === 'admin' || user.role === 'worker') && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Add notes about the work performed to resolve this issue. This is optional.
                </CardDescription>
              </CardHeader>
              <CardContent>
