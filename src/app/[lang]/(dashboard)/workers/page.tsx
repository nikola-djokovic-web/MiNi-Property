...
export default function WorkersPage() {
  ...
  useEffect(() => {
    ...
    const handleWorkerUpdated = (updatedWorker: any) => {
      ...
      const index = allWorkers.findIndex((w) => w.id === updatedWorker.id);
      if (index !== -1) allWorkers[index] = updatedWorker;
    };
  ...
  const handleAddWorker = (newWorkerData: any) => {
    const newWorker = {
      id: `user-worker-${allWorkers.length + 1}`,
      ...newWorkerData,
    };
    eventBus.emit("worker-added", newWorker);
  };
            ...
            <TableBody>
              {workers.map((worker) => {
                const assignedProperties = properties.filter((p) =>
                  worker.assignedPropertyIds?.includes(p.id)
                );
                return (
