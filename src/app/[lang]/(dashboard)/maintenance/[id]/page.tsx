...
import { maintenanceRequests as allMaintenanceRequests, tenants as allTenants, properties as allProperties, workers as allWorkers } from '@/lib/data';
...
export default function MaintenanceDetailPage() {
  ...
  const [request, setRequest] = useState(allMaintenanceRequests.find((r) => r.id === requestId));
  ...
  useEffect(() => {
    const handleRequestUpdated = (updatedRequest: any) => {
      if (updatedRequest.id === requestId) {
        setRequest(updatedRequest);
      }
    };
    const unsub = eventBus.subscribe('maintenance-request-updated', handleRequestUpdated);
    return () => unsub();
  }, [requestId]);
  ...
  const handleStatusChange = (newStatus: "New" | "In Progress" | "Completed") => {
    if (request) {
        ...
        eventBus.emit('maintenance-request-updated', updatedRequest);
  ...
  const canTakeAction = isAssignedToMe || (isAdmin && isAssignedToMe);

  const canClaimRequest = 
    !request.assignedWorkerId &&
    (isAdmin ||
     (isWorker && !!user.assignedPropertyIds?.includes(request.propertyId)));
          ...
          <Card>
            ...
            <CardContent className="space-y-4 text-sm">
                <div className="grid grid-cols-2 gap-x-4 gap-y-6">
                    {isTenant && workerForTenantView ? (
                        <div className="col-span-2 flex items-center gap-3">
                            <Avatar className="h-10 w-10 border">
                                <AvatarImage src={`https://i.pravatar.cc/150?u=${workerForTenantView.id}`} alt={workerForTenantView.name} />
                                <AvatarFallback>{workerForTenantView.name.charAt(0)}</AvatarFallback>
                            </Avatar>
                            <div>
                    ...
                    {request.category && (
                        <div className="space-y-1">
                            <p className="font-medium text-muted-foreground">AI Category</p>
                    ...
                    <div className="col-span-2 flex items-center justify-between">
                         <div className="space-y-1">
                            ...
                            <p>{assignedToUser ? assignedToUser.name : 'Unassigned'}</p>
                        </div>
                      {canChangeAssignment && (
                          <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                  ...
                  <div className="col-span-2 space-y-2">
                     <div className="flex items-center gap-3">
                         {isEditing ? (
                          ...
                          <Badge className={cn(getPriorityClasses(request.priority), "w-28 justify-center")}>
                              <AlertTriangle className="mr-1 h-3 w-3" />
                              {request.priority} Priority
                          </Badge>
                        )}
                        <Badge className={cn(getStatusClasses(request.status), "w-24 justify-center")}>{request.status}</Badge>
                    </div>
                  </div>
                </div>
              </CardContent>
          ...
          {showChat && (
            <Card>
                ...
                <CardContent className="space-y-4">
                    <ScrollArea className="h-64 pr-4" viewportRef={chatViewportRef}>
